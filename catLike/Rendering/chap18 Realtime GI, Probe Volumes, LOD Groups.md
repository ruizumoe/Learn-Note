...
由于使用LOD后可能导致网格发生发生变化，一般光照贴图都是在LOD0的情况下对静态物体进行渲染的，当切换到不同LOD后，可能会出现光照贴图不匹配的情况。因此需要保证简化模型能够正确应用同一套UV（包括UV通道0和UV通道1），那么光照贴图应该能够正确工作。


+ UV通道0（主UV）：用于反照率贴图、法线贴图等材质纹理
+ UV通道1（光照贴图UV）：专门用于光照贴图

如果简化后的模型能正确显示反照率贴图（使用UV通道0），并且UV通道1也保持相同布局，那么光照贴图也应该正确显示。

一个简单的验证方法就是验证不同LOD下的mesh.uv大小是否在[0,1]范围内。
...
LOD0 → LOD1：减少 50-60% 面数

LOD1 → LOD2：减少 60-70% 面数

LOD2 → LOD3：减少 70-80% 面数

LOD3+：减少 80-95% 面数（可能只剩一个平面或立方体）
